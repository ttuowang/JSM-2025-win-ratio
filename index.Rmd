...
`r anicon::faa("link", animate="float", color="#222831")` .fancy[Slides link:] https://ttuowang.github.io/JSM-2025-win-ratio/
...
---
# Restricted Time Win Ratio <span style='font-size:18pt; color:black'> `r icons::fontawesome("arrow-circle-right")` </span> <span style='font-size:23pt; color:black'> Example </span>


.pull-left[
#### Simualted example

- $T_j(1) \sim$ GAMMA(shape = 3, scale = 1), $T_j(0) \sim$ GAMMA(shape =
3, scale = 0.8)

- Censoring: A uniform distribution with mean $r$

- All subjects are censored at time $\tau = 2$ 

- Restricted time win ratio is calculated at $\tau = 2$ while the
win ratio without censoring use the true event times
...
```{r, echo=FALSE, out.width='90%', fig.align='center', fig.cap='The relationship between the mean win ratios and censoring rates'}
knitr::include_graphics('figures/wr1.png')
```
...
<b><span style='color:#ea5455'>Impute the response outcomes up to time $\tau$ </span></b>  <span style='font-size:13pt; color:#707070'> (Wang et al., 2023, 2025) </span> 
...
- Wang, T., Li, Y., and Qu, Y. (2025). Win ratio: from estimands to estimation. *Statistics in Biopharmaceutical Research*
...
<br>
.fancy[Slides link:] https://ttuowang.github.io/JSM-2025-win-ratio/
